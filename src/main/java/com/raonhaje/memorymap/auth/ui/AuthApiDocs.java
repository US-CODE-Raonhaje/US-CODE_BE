...
public interface AuthApiDocs {
    ...
    @Operation(summary = "카카오 로그인", description = "카카오 로그인을 통해 회원 인증을 수행하고, 프론트엔드로 리다이렉트합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "302", description = "리다이렉트 성공"),
            @ApiResponse(responseCode = "401", description = "인증 실패")
    })
    @GetMapping("/kakao/callback")
    void kakaoCallback(@RequestParam("code") String code, HttpServletResponse response) throws IOException;
